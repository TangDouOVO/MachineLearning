...
class DateData:
 ...
 class seq2seq(keras.Model):
    ...
    def encode(self, source):
        ...
        #init_s = [tf.zeros((source.shape[0], self.encoder_rnn_size)), tf.zeros((source.shape[0], self.encoder_rnn_size))]
        res_list = self.encoder_rnn(embedded)
        encoder_hidden = res_list[1][0]
        encoder_state = res_list[1][1]
        #print(encoder_hidden.shape, encoder_state.shape)
        return [encoder_hidden, encoder_state]
    ...
    def train(self, source, target):
        ...
        output, _, _ = self.training_decoder(decoder_embeding_input, initial_state = state, sequence_length = self.seq_len)
        return output.rnn_output
   
   
    def predict(self, source):
        initial_state = self.encode(source)
        ...
        pred_id = np.zeros((self.batch_size, self.target_size), dtype = np.int32)
